...
// --- 1. KONFIGURASI & DATA STATIS (Di Luar Component) ---
...
// --- 2. FUNGSI FORMATTER (Di Luar Component) ---
...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, value }) => {
  ...
  return (
    <text x={x} y={y} fill="#ffffff" textAnchor="middle" dominantBaseline="central" fontSize={11} fontWeight="bold" style={{ textShadow: '0px 1px 3px rgba(0,0,0,0.8)' }}>
      {value}
    </text>
  );
};

// --- 3. SUB-COMPONENTS (Di Luar Component Utama) ---
...
const Badge = ({ status }) => {
  ...
  else if (st.includes("risk") || st.includes("kritis") || st.includes("pending") || st.includes("tunggu") || st.includes("critical") || st.includes("budgetary") || st.includes("prebid")) style = "bg-amber-100 text-amber-700 border-amber-200";
...
// Menerima prop statusText langsung (string)
const LoadBadge = ({ status }) => {
    let color = "bg-emerald-100 text-emerald-700";
    if (status.includes("OVERLOAD")) color = "bg-red-100 text-red-700";
    else if (status.includes("UNDERLOAD")) color = "bg-blue-100 text-blue-700";
    else if (status.includes("IDLE")) color = "bg-slate-100 text-slate-600";
...
const SortableHeader = ({ label, sortKey, currentSort, onSort, align="left" }) => (
    <th className={`px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors group text-${align}`} onClick={() => onSort(sortKey)}>
    ...
    return (
        <div className="flex h-screen w-full items-center justify-center bg-slate-100">
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm border border-slate-200">
                ...
                <p className="text-[10px] text-slate-400 text-center mt-6">Versi 4.0 (Fixed All Issues)</p>
            </div>
        </div>
    );
};

// --- 4. APP COMPONENT UTAMA ---
...
export default function App() {
  ...
  // Settings & Load
  // PENTING: loadSettings didefinisikan di sini agar tidak ReferenceError
  const [loadSettings, setLoadSettings] = useState(() => { 
      try { return JSON.parse(localStorage.getItem('cost_dashboard_load_settings')) || { ongoingOver: 150, ongoingUnder: 80, totalOver: 200, totalUnder: 80 }; } 
      catch { return { ongoingOver: 150, ongoingUnder: 80, totalOver: 200, totalUnder: 80 }; } 
  });
  ...
  const handleSavePasswords = () => {
      if (!adminPassInput || !guestPassInput) return;
      ...
      setPassSaveStatus("Password Tersimpan!");
      setTimeout(() => setPassSaveStatus(""), 3000);
  };

  // Function to update load threshold settings
  const updateLoadThresholds = (key, value) => {
      const newThresholds = { ...loadSettings, [key]: parseInt(value) };
      setLoadSettings(newThresholds);
      localStorage.setItem('cost_dashboard_load_settings', JSON.stringify(newThresholds));
  };
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
      ...
      if (allRows.length > 3) {
          ...
          const parsedData = dataRows.map((row, index) => {
              ...
              for (let i = 0; i < row.length; i++) {
                  let char = row[i]; if (char === '"') { inQuote = !inQuote; continue; } if (char === ',' && !inQuote) { columns.push(currentVal); currentVal = ''; } else { currentVal += char; }
              } columns.push(currentVal);
              if (!columns[1] || columns[1].trim() === "") return null;
              const clean = (val) => val ? val.trim().replace(/^"|"$/g, '') : "";
              if (clean(columns[1]).toLowerCase().includes('total')) return null;
              ...
              else { const backupProgress = parseFloat(clean(columns[13]).replace(/%|,/g, '')); if (!isNaN(backupProgress)) progressVal = (backupProgress <= 1 && backupProgress > 0) ? backupProgress * 100 : backupProgress; }
              const parseDateFromText = (text) => {
                  if (!text) return null;
                  let match = text.trim().match(/^(\d{2})[\.\-\/](\d{2})[\.\-\/](\d{2,4})/);
                  if (!match) match = text.trim().match(/^(\d{2})(\d{2})(\d{2})/);
                  if (match) { let year = parseInt(match[3]); if (year < 100) year += 2000; return new Date(year, parseInt(match[2]) - 1, parseInt(match[1])); } return null;
              };
    ...
    } catch (error) { console.error("Error:", error); setData(MOCK_DATA); setLastSync(new Date()); } 
  ...
  const loadByPic = useMemo(() => {
    ...
    const avgTotalPerPic = totalUniquePics > 0 ? stats.totalProjects / totalUniquePics : 0;
    const avgOngoingPerPic = totalUniquePics > 0 ? stats.ongoingProjects / totalUniquePics : 0;

    data.forEach(p => {
        const picName = p.pic || "Unassigned"; if (picName.toLowerCase().includes("pic utama") || picName.toLowerCase().includes("pic support")) return;
        const cleanName = picName.trim(); const st = (p.status || "").toLowerCase(); const isDone = st.includes('completed') || st.includes('done') || st.includes('selesai') || p.progress >= 100;
    ...
    return Object.values(load).map(p => {
        ...
        const isOngoingOver = p.activeCount > (avgOngoingPerPic * (loadSettings.ongoingOver / 100));
        const isTotalOver = p.count > (avgTotalPerPic * (loadSettings.totalOver / 100));
        const isOngoingUnder = p.activeCount < (avgOngoingPerPic * (loadSettings.ongoingUnder / 100));
        const isTotalUnder = p.count < (avgTotalPerPic * (loadSettings.totalUnder / 100));
  ...
  const filteredProjects = useMemo(() => { 
      ...
      filtered.sort((a, b) => { 
          ...
          if (sortConfig.key === 'last_update_date') { valA = valA ? valA.getTime() : 0; valB = valB ? valB.getTime() : 0; } 
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden relative">
      <aside className="w-64 bg-slate-900 text-white flex-shrink-0 hidden md:flex flex-col shadow-xl z-10">
        <div className="p-6 border-b border-slate-800"><h1 className="text-xl font-bold flex items-center gap-2 text-white"><Briefcase className="text-emerald-400" /> Cost Control</h1><p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">{auth.role === 'admin' ? 'Administrator' : 'Guest Mode'}</p></div>
