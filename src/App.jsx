...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, value }) => {
  ...
  return (
    <text x={x} y={y} fill="#1e293b" textAnchor="middle" dominantBaseline="central" fontSize={11} fontWeight="bold" style={{ textShadow: '0px 0px 3px rgba(255,255,255,0.8)' }}>
      {value}
    </text>
  );
};
    ...
    return (
        <div className="flex h-screen w-full items-center justify-center bg-slate-100">
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm border border-slate-200">
                ...
                <p className="text-[10px] text-slate-400 text-center mt-6">Versi 3.9 (Clean UI)</p>
            </div>
        </div>
    );
};
    ...
    try {
      ...
      if (allRows.length > 3) {
          ...
          const parsedData = dataRows.map((row, index) => {
              ...
              // FILTER OUT HEADERS
              if (clean(columns[1]).toUpperCase() === 'JUDUL PEKERJAAN') return null;
