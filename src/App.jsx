...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, value }) => {
  ...
  return <text x={x} y={y} fill="#1e293b" textAnchor="middle" dominantBaseline="central" fontSize={10} fontWeight="bold" style={{ textShadow: '0px 1px 3px rgba(255,255,255,0.8)' }}>{value}</text>;
};
...
const LoadBadge = ({ status }) => {
    ...
    else if (status && (status.includes("UNDERLOAD") || status.includes("IDLE"))) color = "bg-blue-100 text-blue-700";
    ...
    <Card className={`p-4 border-l-4 ${colorClass} flex flex-col justify-between h-full`}>
        <div className="flex justify-between items-start mb-2">
            <div className="overflow-hidden pr-2">
                ...
                <h3 className="text-lg md:text-xl font-bold text-slate-800 mt-1 whitespace-nowrap overflow-hidden text-ellipsis" title={value}>{value}</h3>
            </div>
...
const SortableHeader = ({ label, sortKey, currentSort, onSort, align="left" }) => (
    <th className={`px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors group text-${align} sticky top-0 z-20 bg-slate-50 shadow-sm`} onClick={() => onSort(sortKey)}>
    ...
    return (
        <div className="flex h-screen w-full items-center justify-center bg-slate-100">
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm border border-slate-200">
                ...
                <p className="text-[10px] text-slate-400 text-center mt-6">Versi 3.7 (Scrollable Breakdown Notes)</p>
            </div>
        </div>
    );
};
...
export default function App() {
  ...
  const handleLogin = (role) => { const newAuth = { isAuth: true, role }; setAuth(newAuth); localStorage.setItem('cost_dashboard_auth_state', JSON.stringify(newAuth)); if (role === 'guest') setProfitViewMode('owner'); setAdminPassInput(passwords.admin); setGuestPassInput(passwords.guest); };
  const handleLogout = () => { setAuth({ isAuth: false, role: null }); localStorage.removeItem('cost_dashboard_auth_state'); };
    ...
    try {
      ...
      if (allRows.length > 3) {
          ...
          const parsedData = dataRows.map((row, index) => {
              ...
              for (let i = 0; i < row.length; i++) { let char = row[i]; if (char === '"') { inQuote = !inQuote; continue; } if (char === ',' && !inQuote) { columns.push(currentVal); currentVal = ''; } else { currentVal += char; } } columns.push(currentVal);
              ...
              const specificStatus = clean(columns[13]); 
              const picSupport = clean(columns[5]);

              return {
                  id: index, project_name: clean(columns[1]), department: clean(columns[2]), owner: clean(columns[3]) || "General", pic: clean(columns[4]), pic_support: picSupport,
                  progress: progressVal, tindak_lanjut: colProgressText, last_update_date: lastUpdateDate, status: rawStatus, specific_status: specificStatus,
